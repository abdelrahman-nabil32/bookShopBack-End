...
const registration = async (req, res) => {
    ...
    if((!fullName)||(!username)||(!email)||(!password)||(!phoneNumber)||(!address)||(!role)){
        return res.status(400).json({status:"FAIL",message:"fullName,username,email,password,phoneNumber,address and role all must not be empty"})
    }else if(/\s/.test(username)){
        return res.status(400).json({status:"FAIL",message:"username must not have spaces"})
    }else if(!validator.isEmail(email)){
        return res.status(400).json({status: 'FAIL' , message: 'email must be in the construction of emails'});
    ...
    }else if((role!=="customer")&&(role!=="seller")){   // may be changes here
        return res.status(400).json({status:"FAIL",message:"user role should be whether seller or customer"});
    }
    //--------------------------------------
    try {
        //checking whether the email is already existent or not
        const checkedUser = await UserModel.findOne({"email":email},{"password":false,"refreshToken":false});
        if(checkedUser){
            return res.status(400).json({status:"FAIL",message:"this email is already existent"});
        }
...
const login = async(req,res)=>{
    ...
    if((!email)||(!password)){
        return res.status(400).json({status:"FAIL",message:"email and password shouldn't be empty"});
    }
    try {
        //email checking
        ...
        const refreshToken = jwt.sign({tokenUserID:checkedUser["_id"]}, process.env.REFRESH_TOKEN_SECRET_KEY, { expiresIn: '1d'});
        const accessToken = jwt.sign({   // may be changes here
            tokenUserID:checkedUser["_id"],
            fullName:checkedUser.fullName,
            username:checkedUser.username,
            role:checkedUser.role,
            email
        }, process.env.ACCESS_TOKEN_SECRET_KEY, { expiresIn: '1h'});     // may be changes here
...
try {
    ...
    if(!user){
      return  res.status(404).json({status:"FAIL",message:"Invalid refresh token"})
    }
    user.refreshToken = "";
    await user.save();    
...
const accessTokenValidation = (req, res ,next) => {
    ...
    if(!accessToken)
     return   res.status(401).json({status:"FAIL",message:"access token is required"});

    try {
        const verifiedAccessToken =  jwt.verify(accessToken,process.env.ACCESS_TOKEN_SECRET_KEY);
        req.user = {
...
const getNewAccessTokenByRefreshToken =async (req, res)=>{
    ...
    try {
        const verifiedRefreshToken = jwt.verify(refreshToken,process.env.REFRESH_TOKEN_SECRET_KEY);
        const checkedUser = await UserModel.findById(verifiedRefreshToken.tokenUserID);
        ...
        const newAccessToken = jwt.sign({   // may be changes here
            tokenUserID:checkedUser["_id"],
            fullName:checkedUser.fullName,
            username:checkedUser.username,
            role:checkedUser.role,
        ...
        res.status(201).json({status:"SUCCESS",data:{"accessToken":newAccessToken}});
    } catch (error) {
    return res.status(403).json({status:"ERROR",message:"invalid refresh token"}); 
    }
}
